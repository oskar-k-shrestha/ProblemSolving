...
public class KthMissingPositiveNumber {
    public static void main(String[] args) {
        ...
        System.out.println(k + "th missing number is : " + findMissingNumber(arr, k));
        System.out.println(k + "th missing number is : " + findMissingNumber1(arr, k));
     ...
     * Trick: Start with the assumption that K is the smallest missing number.
     * Iterate through the array:
     * - If the current number is ≤ K, it means K cannot be the missing number, so increment K.
     * - As soon as you find a number greater than K, you know that K is the smallest missing number.
     * This works because every number ≤ K must be accounted for in order to validate K as the answer.
     */
     ...
     * Space complexity : O(1)
     * Trick:
     * If there were no missing numbers, the array would look like: [1, 2, 3, 4, ...]
     * At any index `mid`, the number of missing elements to the left of `arr[mid]` can be calculated as:
     *     missing = arr[mid] - (mid + 1)
     * This works because at index `mid`, we expect the value to be `mid + 1` in a perfect sequence.
     * Binary Search Logic:
     * - If missing <= k → move right (we need more missing numbers).
     * - If missing > k → move left (we've gone too far).
     * After the loop ends:
     * - `high` points to the last index where missing count was still ≤ k.
     * - At that point, we've seen (high + 1) real numbers.
     * So, the k-th missing number must be:
     *     k + high + 1
     */
    public static int findMissingNumber1(int[] arr, int k) {
        int low = 0;
